...
from mask_features_computation.features_extractor_mp import FeaturesExtractor
import torch
import os
...
def process_scene(args):
    ctx, masks_path, device, out_folder = args
    ...
    with print_lock:
        print(f"[INFO] Processing scene {scene_num_str}: Total image count: {total_images}. Selected {len(indices)} images with frequency {ctx.openmask3d.frequency}")
...
def main(ctx: DictConfig):
    ...
    device = get_free_gpu(7000, device_num=0) if torch.cuda.is_available() else device
    ...
    args_list = [(ctx, masks_path, device, out_folder) for masks_path in masks_paths]

    # Use multiprocessing to process scenes in parallel
    num_worker=16 #cpu_count()
    print(f"[INFO] Multiprocessing activated with {num_worker} workers")
    with Pool(num_worker) as pool:
        pool.map(process_scene, args_list)
