...
# Initialize a global lock for print statements
print_lock = Lock()
...
class PointProjector:
    ...
    def get_visible_points_view(self):
        ...
        for i, idx in tqdm(enumerate(indices), desc="Processing Masks", position=0, leave=True): # for each view
            # *******************************************************************************************************************
    ...
    def get_visible_points_in_view_in_mask(self):
        ...
        for i in tqdm(range(num_view), desc="Processing Masks", position=0, leave=True):
...
class FeaturesExtractor:
    ...
    def extract_features(self, topk, multi_level_expansion_ratio, num_levels, num_random_rounds, num_selected_points, save_crops, out_folder, optimize_gpu_usage=False):
        ...
        for mask in tqdm(range(num_masks), desc="Processing Masks", position=0, leave=True): # for each mask 
            images_crops = []
