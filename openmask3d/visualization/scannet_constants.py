...
def make_color_label():
    # Create a new image with a white background
    width, height = 180, 800  # Adjust based on the number of classes and desired spacing
    ...
    x_position = 10
    y_position = 10
    box_height = 18  # Adjust based on font size
    spacing = 4  # Space between boxes

    for label, color in SCANNET_COLOR_MAP_40.items():
        # Draw the color box
        draw.rectangle([x_position, y_position, x_position + box_height + 23, y_position + box_height], fill=(int(color[0]), int(color[1]), int(color[2])))
        draw.text((x_position + box_height + 30, y_position), CLASS_LABELS_40[label], fill='black', font=font)
        y_position += box_height + spacing

    # Save the image
    image_path = '/home/tidy/PycharmProjects/SegmentAnything3D/openmask3d/openmask3d/visualization/scannet_label.png'
